...
finalHPIDBData <- fread("~/Documents/Project/Programming/Ppi/Data/HPIDBDATAforEnrichmentAnalysis.csv", header = T, sep = ",")
head(finalHPIDBData)
str(finalHPIDBData)
...
head(positiveSet)
sum(positiveSet$Ensembl_accession %in% finalHPIDBData$`ensembl_gene_id`)
