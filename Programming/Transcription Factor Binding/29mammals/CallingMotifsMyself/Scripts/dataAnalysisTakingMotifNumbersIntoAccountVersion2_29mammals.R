...
loadLibraryAndData <- function(reloadFromSource = FALSE) {
  ...
  scoresOverRows <- function(wholeGenomeDataOneLine, fisherTests, accountForTFBSCount = TRUE, amountTFBSMotifsForNominalScore = 1) {
  ...
  informativeTFBS           <- fisherTests[fisherTests$qValue <= 0.10, , drop = FALSE]
  ...
  print( informativeTFBS[informativeTFBS$sampleEstimate >=1, ])
  informativeTFBS[informativeTFBS$sampleEstimate <1, ]$score = -informativeTFBS[informativeTFBS$sampleEstimate <1, ]$sampleEstimate / informativeTFBS[informativeTFBS$sampleEstimate <1, ]$qValue
  informativeTFBS[informativeTFBS$sampleEstimate >=1, ]$score = informativeTFBS[informativeTFBS$sampleEstimate >=1, ]$sampleEstimate / informativeTFBS[informativeTFBS$sampleEstimate >=1, ]$qValue
  ...
  for(rows in 1:nrow(wholeGenomeDataOneLine)) {
    ...
    for(bindingSites in 1:length(logicalPresence)) {
      ...
      if(logicalPresence[bindingSites] == TRUE) {
        ...
        if(accountForTFBSCount == TRUE) {
          
          TFBSCount  <- countVectorTFBS[informativeTFBS$TFBS[bindingSites]]
          valueToAdd <- informativeTFBS$score[bindingSites] * (TFBSCount * (1/amountTFBSMotifsForNominalScore))
          #print(valueToAdd)
  ...
  if (runselection == TRUE) {
    ...
    saveRDS(finalScoresWholeGenome, file = "/home/dieter/Documents/Project/Programming/Transcription Factor Binding/29mammals/CallingMotifsMyself/Data/FinalDfAndUnifiedMotifFile/Generalised29mammalsMotifsTotalScoreWholeGenome.rds")
     }
  if (runselection == FALSE) {
    finalScoresWholeGenome <- readRDS(file = "/home/dieter/Documents/Project/Programming/Transcription Factor Binding/29mammals/CallingMotifsMyself/Data/FinalDfAndUnifiedMotifFile/Generalised29mammalsMotifsTotalScoreWholeGenome.rds")
    
      }
  str(finalScoresWholeGenome)
  head(finalScoresWholeGenome$scoresWholeGenome, 30)
  tail(finalScoresWholeGenome$scoresWholeGenome, 30)
