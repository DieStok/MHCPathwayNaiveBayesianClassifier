...
#i.e. STAT_9 and STAT_12 both become simply STAT
...
#create new data: non-unique (i.e. one gene with GABp_2 and GABP_3 will now contain GABP 2 times)
...
head(regexSplitList)
equalMotifsOneLine <- unlist(lapply(regexSplitList, FUN = function(x) { motifs <- x[,2];paste0(motifs, collapse = "|")}))
ensemblIds <- oldDataOneLine[,1]
newMotifDataOneLine <- data.table(EnsemblId = ensemblIds, TranscriptionFactorBindingSites = equalMotifsOneLine)
names(newMotifDataOneLine) <- c("EnsemblId", "TranscriptionFactorBindingSites")
head(newMotifDataOneLine)
head(oldDataOneLine)
write_csv(newMotifDataOneLine, "~/Documents/Project/Programming/Transcription Factor Binding/29mammals_unique_tfbs_per_gene_one_line_motifversionsdeleted_nonunique_DieterStoker_01_07_2017.txt")

#same for the multiple line data
oldDataMultipleLines <- fread("~/Documents/Project/Programming/Transcription Factor Binding/29mammals_unique_tfbs_per_gene_per_line.txt", header = FALSE)
head(oldDataMultipleLines)
equalMotifsMultipleLines <- str_match_all(oldDataMultipleLines[,2], "([a-zA-Z0-9-:.]+)(_\\d+){0,1}")[[1]][,2]
#a random "c" pops up as a motif, while it is not in the data. I remove it here
equalMotifsMultipleLines <- equalMotifsMultipleLines[-1]
#check if equal size
length(equalMotifsMultipleLines) == nrow(oldDataMultipleLines)

newMotifDataMultipleLines <- data.table(EnsemblId = oldDataMultipleLines[,1],
                                        TranscriptionFactorBindingSites = equalMotifsMultipleLines)
names(newMotifDataMultipleLines) <- c("EnsemblId", "TranscriptionFactorBindingSites")
head(newMotifDataMultipleLines)
write_csv(newMotifDataMultipleLines, "~/Documents/Project/Programming/Transcription Factor Binding/29mammals_unique_tfbs_per_gene_multiple_lines_motifversionsdeleted_nonunique_DieterStoker_01_07_2017.txt")
...
loadLibraryAndData <- function(reloadFromSource = FALSE) {
  ...
  if(reloadFromSource == TRUE) {
    ...
    tFTableOneLine <- fread(tk_choose.files(default = "~/Documents/Project/Programming/Transcription Factor Binding/29mammals_unique_tfbs_per_gene_one_line_motifversionsdeleted_nonunique_DieterStoker_01_07_2017.txt",
    ...
    tFTableMultipleLines <- fread(tk_choose.files(default = "~/Documents/Project/Programming/Transcription Factor Binding/29mammals_unique_tfbs_per_gene_multiple_lines_motifversionsdeleted_nonunique_DieterStoker_01_07_2017.txt",
    ...
    saveRDS(tFTableOneLine, file = paste(currentDir, "/tFTableOneLineNonUniqueMotifs.rds", sep=""))
    saveRDS(tFTableMultipleLines, file = paste(currentDir, "/tFTableMultipleLinesNonUniqueMotifs.rds", sep=""))
    saveRDS(MHCGeneTable, file = paste(currentDir, "/MHCGeneTable.rds", sep=""))
    
  } else if (reloadFromSource == FALSE) {
    
    tFTableOneLine <- readRDS(file = paste(currentDir, "/tFTableOneLineNonUniqueMotifs.rds", sep=""))
    tFTableMultipleLines <- readRDS(file = paste(currentDir, "/tFTableMultipleLinesNonUniqueMotifs.rds", sep=""))
...
computeFishersTest = function(wholeGenomeTFBSCountData, positiveSetTFBSCountData, run = FALSE) {
  ...
  if(run == FALSE) {
    ...
    fisherDataFrame <- readRDS(file = paste(getwd(), "/fisherDataFrameNonUniqueMotifs.rds", sep = ""))
    ...
    saveRDS(fisherDataFrame, file = paste(getwd(), "/fisherDataFrameNonUniqueMotifs.rds", sep = ""))
    return(fisherDataFrame)
  }
...
if (runselection == TRUE) {
  ...
  saveRDS(finalScoresWholeGenome, file = paste(getwd(), "/FinalScoreWholeGenomeNonUniqueMotifs.rds",sep=""))
}
if (runselection == FALSE) {
  finalScoresWholeGenome <- readRDS(file = paste(getwd(), "/FinalScoreWholeGenomeNonUniqueMotifs.rds", sep=""))
}
...
plotGraphData <- function(plottingData, run = TRUE) {
  ...
  if(run == TRUE) {
    ...
    annotatedPlot
    ggsave(plot = annotatedPlot, filename = "TFBSOverrepresentationPlot_binaryNonUniqueMotifs.pdf", device = "pdf", dpi = 600)
  ...
  if (run == TRUE) {
    ...
    for(relevantTFBS in 1:length(finalScoresWholeGenome$informativeTFBS$TFBS)) {
      ...
      if (relevantTFBS == 1) {
        ...
        write.csv(genelist[[relevantTFBS]], "PositiveGenesWithTFBSNonUniqueMotifs.csv")
        
      }
      
      else if (relevantTFBS > 1) {
        
        write.table(genelist[[relevantTFBS]], "PositiveGenesWithTFBSNonUniqueMotifs.csv", append = TRUE, sep = ",", col.names = FALSE)
...
outputEnsIdsWithTFBS <- function(run = FALSE) {
  ...
  if(run == TRUE) {
    ...
    write.csv(dfWGDEnsmblIdsPerTFBS, "EnsemblIdsWGDNonUniqueMotifs.csv")
